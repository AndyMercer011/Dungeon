...
static void io_shooting_display(dungeon *d, pair_t cursor){
	pair_t pos;
  //uint32_t color;
  uint32_t illuminated;
...
void io_range_shoot(dungeon *d){
	...
	//mvprintw(0, 0,
    //       "Choose a location.  'r' to shoot the monster, careful choose your target, or you will miss the shoot");
  ...
  * not otherwise used.                                                 */
    ...
    case 'y':
    case KEY_HOME:
      if (dest[dim_y] != 1 && dest[dim_y]-1 >= (d->PC->position[dim_y]-3)) {
        dest[dim_y]--;
      }
      if (dest[dim_x] != 1 && dest[dim_x]-1 >= (d->PC->position[dim_x]-3)) {
        dest[dim_x]--;
      }
      //mvaddch(dest[dim_y] + 1, dest[dim_x], '*');
      mvaddch(dest[dim_y] + 1, dest[dim_x], '*');
    ...
    case KEY_PPAGE:
      ...
      if (dest[dim_x] != DUNGEON_X - 2 && dest[dim_x]+1 <= (d->PC->position[dim_x]+3)) {
        dest[dim_x]++;
      }
      mvaddch(dest[dim_y] + 1, dest[dim_x], '*');
    refresh();
    ...
    case 'n':
    case KEY_NPAGE:
      if (dest[dim_y] != DUNGEON_Y - 2 && dest[dim_y]+1 <= (d->PC->position[dim_y]+3)) {
        dest[dim_y]++;
      }
      if (dest[dim_x] != DUNGEON_X - 2 && dest[dim_x]+1 <= (d->PC->position[dim_x]+3)) {
        dest[dim_x]++;
  ...
  } while (c != 'r' && c != 'R' && c != 27 /* ESC */ && c != 's');
  if(c == 's'){
  	int i,j;
  	for(i = dest[dim_y]-1;i<=dest[dim_y]+1;i++){
  		for(j = dest[dim_x]-1;j<=dest[dim_x]+1;j++){
  			if (d->character_map[i][j] != NULL && d->character_map[i][j] != d->PC){
  				uint dam = rand() %10;
  				character *t = d->character_map[i][j];
  				if(d->character_map[i][j]->hp <= dam){
				  
  			
            io_queue_message("The %s dies.", t->name);
           	t->hp = 0;
            t->alive = 0;
            character_increment_dkills(d->PC);
            character_increment_ikills(d->PC, (character_get_dkills(t) +
                                                                   character_get_ikills(t)));
            if (t != d->PC) {
                d->num_monsters--;
            }
            charpair(t->position) = NULL;
  			}
  			else{
  				io_queue_message("You deal %d damage to the %s", dam, t->name);
  ...
  if (c == 'r'){
  	if (d->character_map[dest[dim_y]][dest[dim_x]] != NULL){
  		character *t = d->character_map[dest[dim_y]][dest[dim_x]];
  		uint dam = d->PC->eq[2]->roll_dice();
  		if (d->character_map[dest[dim_y]][dest[dim_x]]->hp <= dam) {
            io_queue_message("The %s dies.", t->name);
            t->hp = 0;
            t->alive = 0;
            character_increment_dkills(d->PC);
            ...
            io_queue_message("You deal %d damage to the %s", dam, t->name);
            d->character_map[dest[dim_y]][dest[dim_x]]->hp = d->character_map[dest[dim_y]][dest[dim_x]]->hp - dam;
        }
  		
	  }
  	
  }
  // destory every monster!
  if(c == 'R'){
  	int i,j;
...
void io_handle_input(dungeon *d)
{
  ...
  do {
    ...
    case 'r':
      if(d->PC->eq[2] != NULL){
      	mvprintw(0, 0,
           "Choose your target, use R to throw holy grenade, r to shoot at some enemy , or s to throw spell");
      	io_range_shoot(d);
      	fail_code = 0;
	  }
	  else{
	  	fail_code = 0;
	  	mvprintw(0, 0,
           "You don't have a ranged weapon, try find one, but you can use your spell 's'");
           // for test;
        io_range_shoot(d);
        refresh();
	  }
